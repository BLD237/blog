...
include 'connect.php';
$id = $_GET['r'];
$sql = "SELECT `comments.comment`, `users.username`  FROM `comments`  INNER JOIN users ON `comments.user_id`    WHERE `blog_id` = $id";
$results = $dbconnect -> query($sql);
var_dump($results);
die();
while($row = $results -> fetch_assoc()){
    $username = $row['comments.comment'];
    $comment = $row['users.username'];                    
      echo"
